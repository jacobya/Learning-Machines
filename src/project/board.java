...
public class board {
	...
	public board() {
		...
		for (int i = 0; i < 4; i++) {
			for (int j = 0; j < 12; j++) {
				...
				optionMoveFull.add(new node(i, j));
			}
		}
	 ...
	 */
	private  boolean checkWin(node move) {
		boolean win = false;
	...
	private  double TDNN(String type) {
		...
		for (int i = 0; i < hiddenA; i++) {
			...
			for (int j = 0; j < inputA; j += 2) {
				double temp = amount;
				if (j < 96) {
					if ((Board[(j / 2) % 4][(j / 2) / 12].getPiece().equals(type))) {
						amount += inWeight[i][j];
						amount += inWeight[i][j + 1] * 0;
					} else if (Board[(j / 2) % 4][(j / 2) / 12].getPiece()
							.equals(otherType)) {
	...
	public boolean StartTD(String type) {
		...
		double[] score = new double[optionMoveFull.size()];
		//System.out.println(optionMoveFull.size());
		score[0] = -10000;
		int max = 0;

		for (int i = 0; i < optionMoveFull.size(); i++) {
			double temp = TDOUT(optionMoveFull.get(i).getRing(), optionMoveFull.get(i).getLine(), type);
			if (score[max] < temp) {
				max = i;
			}
			score[i] = temp;
...
//		}
		add(type, optionMoveFull.get(max).getRing(), optionMoveFull.get(max).getLine());

		double scoreAfter = TDNN(type);
		error.writer(scoreAfter-scoreBefore);
		optionMoveFull.get(max).setPiece(type);
		node temp = optionMoveFull.get(max);
