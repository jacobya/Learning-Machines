...
public class MiniMax {
	...
	private blanket expand(blanket in, String move, boolean minimax,
			int depthTotal) {
		int num = 1000000000;
		...
		if (in.depth == 0) {
			...
			for (int i = 0; i < in.turns.size(); i++) {
				if (i == root.turns.size()
						&& move.equals(root.current.getPiece())) {
					in.score = in.currentBoard
							.score(in.turns.get(i).getPiece());
					return in;
				}
				if (minimax == false) {
					...
					temp.currentBoard.add(move, thing.getRing(),
							thing.getLine(), false);
					in.pointers.add(temp);
					if (in.turns.get(i).getPiece().equals("N")
							|| !in.currentBoard.checkWin(in.turns.get(i))) {
						blanket theThing = expand(in.pointers.get(i),
								otherType, minimax, depthTotal);
						score.add(theThing);
					} else if (in.turns.get(i).getPiece().equals("N")
							|| i == root.turns.size()
							&& move.equals(root.current.getPiece())) {
					...
					temp.currentBoard.add("N", temp.current.getRing(),
							temp.current.getLine(), false);
